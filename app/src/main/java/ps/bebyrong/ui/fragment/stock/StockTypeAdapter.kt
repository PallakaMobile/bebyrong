...
import ps.bebyrong.databinding.FragmentStockTypeItemBinding
 ...
 */
class StockTypeAdapter : RecyclerView.Adapter<StockTypeAdapter.StockTypeHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): StockTypeHolder =
            StockTypeHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.fragment_stock_type_item, parent, false))
    ...
    fun updateStockTypeAdapter(dataDataCategoryList: List<ResponseStock.DataItem>) {
        this.dataStockList = dataDataCategoryList
        notifyDataSetChanged()
    }

    inner class StockTypeHolder(private val binding: FragmentStockTypeItemBinding) : RecyclerView.ViewHolder(binding.root) {
