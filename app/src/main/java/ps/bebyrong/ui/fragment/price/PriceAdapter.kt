...
import ps.bebyrong.databinding.FragmentPriceItemBinding
...
class PriceAdapter : RecyclerView.Adapter<PriceAdapter.PriceHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PriceHolder =
            PriceHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.fragment_price_item, parent, false))
    ...
    inner class PriceHolder(private val binding: FragmentPriceItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindDataPrice(dataPrice: ResponsePrice.DataItem) = with(itemView) {
            ...
            for (loops in dataPrice.pasar?.indices ?: throw NullPointerException()) {
                ...
                tv.text = dataPrice.pasar[loops].nama.toString()
