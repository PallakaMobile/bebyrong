...
import ps.bebyrong.databinding.FragmentCommodityItemBinding
...
class CommodityAdapter : RecyclerView.Adapter<CommodityAdapter.CommodityHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CommodityHolder =
            CommodityHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.fragment_commodity_item, parent, false))

    override fun onBindViewHolder(holder: CommodityHolder, position: Int) = holder.bindDataCommodity(dataDataCommodityFoodList[position])

    override fun getItemCount() = if (::dataDataCommodityFoodList.isInitialized) dataDataCommodityFoodList.size else 0

    fun updateCommodityAdapter(dataDataCommodityFoodList: List<ResponseFoodList.DataItem>) {
        this.dataDataCommodityFoodList = dataDataCommodityFoodList
        notifyDataSetChanged()
    }

    inner class CommodityHolder(private val binding: FragmentCommodityItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindDataCommodity(dataDataCommodity: ResponseFoodList.DataItem) = with(itemView) {
            ...
            setOnClickListener {
                Bundle().apply {
                    e { dataDataCommodity.id.toString() }
